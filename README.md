...
- 本地拉黑 UID：按 UP 主/用户 UID 隐藏相关视频、评论、用户卡片，并拦截命中的个人主页和视频页；评论区会深度识别用户链接和 UID 属性。
- 用户名关键词：按用户名关键词隐藏内容、评论和用户页面内容；适配新版评论区，命中主评论作者时会隐藏整条评论及其下方回复。
- 视频标题关键词：隐藏标题命中的视频卡片。
- 评论关键词：隐藏评论内容命中的评论，包含 iframe/子框架注入和文本命中兜底扫描。
